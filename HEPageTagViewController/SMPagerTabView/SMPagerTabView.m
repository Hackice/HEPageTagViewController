...
- (void)selectTabWithIndex:(NSUInteger)index animate:(BOOL)isAnimate {
    ...
    vc.view.frame = CGRectMake(self.bodyScrollView.width * index, 0, self.bodyScrollView.width, self.bodyScrollView.height);
...
- (UIView *)tabView {
    if (!_tabView) {
        ...
        UIView *sepLine = [[UIView alloc]initWithFrame:CGRectMake(0, _tabFrameHeight-0.5, self.width, 0.5)];
        sepLine.backgroundColor = [UIColor lightGrayColor];
        [self.tabView addSubview:sepLine];
...
- (UIColor *)tabButtonTitleColorForNormal {
    if (!_tabButtonTitleColorForNormal) {
        self.tabButtonTitleColorForNormal = [UIColor lightGrayColor];
    }
...
- (UIView *)selectedLine {
    if (!_selectedLine) {
        ...
        _selectedLine.backgroundColor = self.tabButtonTitleColorForSelected;
...
- (void)setDisableScroll:(BOOL)disableScroll
{
    ...
    self.bodyScrollView.scrollEnabled = !disableScroll;
}
@end
